...
import kotlinx.android.synthetic.main.thumbnail.view.ivImage
...
class ThumbnailAdapter(private val data: List<ThumbnailItem>, private val onClick: (Int) -> Unit) :
    ...
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): ViewHolder {
        return ViewHolder(
            LayoutInflater.from(p0.context).inflate(R.layout.thumbnail, p0, false),
            onClick
        )
    }
...
@Parcelize
data class ThumbnailItem(val name: String, val thumb: Int, val desc: String, val location: String) :
    Parcelable {
    fun getImages(ctx: Context): List<Int> {
        return (1..10).map {
            ctx.resources.getIdentifier(
                "${name.toLowerCase()}_$it",
                "drawable",
                ThumbnailItem::class.java.`package`?.name
            );
        }
    }
}